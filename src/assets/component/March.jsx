...
const fetchData = async () => {
  const response = await axios.get("https://fishing-project-server.vercel.app/marchProject");
  return response.data;
};

export const March = () => {
  const { data = [], refetch,isLoading,error } = useQuery({
    queryKey: ["marchProject"], // Fixed incorrect queryKey
    queryFn: fetchData,
  });
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://fishing-project-server.vercel.app/marchProject/${id}`)
          .then((response) => {
